...
class Deck:
    ...
    def parse_text_decklist(self, path):
        ...
        nonbasics = set(self.main + self.side) - set(['Mountain', 'Island', 'Forest', 'Swamp', 'Plains'])
    ...
    def determine_colors(self, magic_cards):
        ...
        nonland_colors = [''.join(magic_cards[card]['color_identity']) for card in self.main if 'Land' not in magic_cards[card]['type_line']]
